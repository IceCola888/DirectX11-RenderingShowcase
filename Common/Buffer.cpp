...
Buffer::Buffer(ID3D11Device* d3dDevice, D3D11_SUBRESOURCE_DATA* pData, const CD3D11_BUFFER_DESC& bufferDesc)
    : m_ByteWidth(bufferDesc.ByteWidth)
{
    if (bufferDesc.MiscFlags & D3D11_RESOURCE_MISC_DRAWINDIRECT_ARGS) {
        ...
        d3dDevice->CreateBuffer(&bufferDesc, &InitArgsData, m_pBuffer.GetAddressOf());
    }
    else {
        d3dDevice->CreateBuffer(&bufferDesc, pData, m_pBuffer.GetAddressOf());
    }
    if (bufferDesc.BindFlags & D3D11_BIND_UNORDERED_ACCESS) {
        d3dDevice->CreateUnorderedAccessView(m_pBuffer.Get(), nullptr, m_pUnorderedAccess.GetAddressOf());
    }
...
Buffer::Buffer(ID3D11Device* d3dDevice,
