...
// 「\」を使用して、プログラムに対してこの行がまだ終了しておらず、次の行の内容をこの行に結合することを指示する
// 2つの「##」で囲まれた部分は、引数の形式からコードに変換される
...
enum ShaderFlag {
    // D3D11_SHADER_VERSION_TYPE に基づいて 1 を左シフトして取得する
    PixelShader = 0x1,
    VertexShader = 0x2,
    GeometryShader = 0x4,
    HullShader = 0x8,
...
struct ConstantBufferVariable : public IEffectConstantBufferVariable
{
    ...
    {
        // 提供されたデータポインタが空であるか
        // またはバイトオフセットがバッファの幅を超えた場合、関数は直接リターンする
        // 境界外書き込みを防止
        if (!data || byteOffset > byteWidth)
            return;

        // byteCountを調整し、バッファの末尾までのみカバーするようにする
...
struct VertexShaderInfo
{
    ...
    uint32_t cbUseMask = 0;      // この頂点シェーダーが使用する定数バッファーを示すマスク
    uint32_t ssUseMask = 0;      // この頂点シェーダーが使用するサンプラーステートを示すマスク
    uint32_t unused = 0;         // 現在未使用のメンバー
    uint32_t srUseMasks[4] = {}; // この頂点シェーダーが使用するシェーダーリソースを示すマスク
    std::unique_ptr<CBufferData> pParamData = nullptr;// このシェーダーに関連する定数バッファーのデータを格納
    std::unordered_map<size_t, std::shared_ptr<ConstantBufferVariable>> params;// 定数バッファー内の各変数を管理
};
...
struct EffectPass : public IEffectPass
{
    ...
    // シェーダーパラメータの定数バッファデータ（定数バッファは作成しない）
...
public:
    ...
    // すべてのリソースとリフレクション情報をクリア
    void Clear();
    // Blob からシェーダーを作成し、識別名を指定
    HRESULT CreateShaderFromBlob(std::string_view name, ID3D11Device* device, uint32_t shaderFlag,
        ID3DBlob* blob);

public:
    std::unordered_map<size_t, std::shared_ptr<EffectPass>> m_EffectPasses;			                // レンダリングパス

    std::unordered_map<size_t, std::shared_ptr<ConstantBufferVariable>> m_ConstantBufferVariables;  // 定数バッファ変数
    std::unordered_map<uint32_t, CBufferData> m_CBuffers;											// 定数バッファの一時キャッシュデータ
    std::unordered_map<uint32_t, ShaderResource> m_ShaderResources;									// シェーダーリソース
    std::unordered_map<uint32_t, SamplerState> m_Samplers;											// サンプラー
    std::unordered_map<uint32_t, RWResource> m_RWResources;											// 読み書き可能リソース

    std::unordered_map<size_t, std::shared_ptr<VertexShaderInfo>> m_VertexShaders;	                // 頂点シェーダー
    std::unordered_map<size_t, std::shared_ptr<HullShaderInfo>> m_HullShaders;		                // ハルシェーダー
    std::unordered_map<size_t, std::shared_ptr<DomainShaderInfo>> m_DomainShaders;	                // ドメインシェーダー
    std::unordered_map<size_t, std::shared_ptr<GeometryShaderInfo>> m_GeometryShaders;              // ジオメトリシェーダー
    std::unordered_map<size_t, std::shared_ptr<PixelShaderInfo>> m_PixelShaders;		            // ピクセルシェーダー
    std::unordered_map<size_t, std::shared_ptr<ComputeShaderInfo>> m_ComputeShaders;	            // コンピュートシェーダー

    std::filesystem::path m_CacheDir;                                                               // キャッシュパス
    bool m_ForceWrite = false;                                                                      // 強制コンパイル後キャッシュ
};
...
HRESULT EffectHelper::Impl::UpdateShaderReflection(std::string_view name, ID3D11Device* device, ID3D11ShaderReflection* pShaderReflection, uint32_t shaderFlag)
{
    ...
    for (uint32_t i = 0;; ++i)
    {
        ...
        //    LPCSTR                   Name;			// シェーダーリソース名
        //    D3D_SHADER_INPUT_TYPE    Type;			// リソースタイプ
        //    UINT                     BindPoint;		// 指定された入力スロットの開始位置
        //    UINT                     BindCount;		// 配列の場合、占有するスロット数
        //    UINT                     uFlags;		    // D3D_SHADER_INPUT_FLAGS列挙の複合
        //    D3D_RESOURCE_RETURN_TYPE ReturnType;	    // 
        //    D3D_SRV_DIMENSION        Dimension;		// シェーダーリソースの種類
        //    UINT                     NumSamples;	    // テクスチャの場合はMSAAサンプル数、それ以外は0xFFFFFFFF
        //} D3D11_SHADER_INPUT_BIND_DESC;

        // 注意：シェーダーコード内で一度も現在のシェーダーによって使用されていないリソースは列挙されない
        // そのため、シェーダーデバッグやシェーダーリフレクション時にそれらを見ることはできない
        // しかし、逆アセンブルではその変数がunusedとマークされている可能性がある

        D3D11_SHADER_INPUT_BIND_DESC sibDesc;
        hr = pShaderReflection->GetResourceBindingDesc(i, &sibDesc);
        // 変数を読み取った後に失敗するが、これは呼び出しの失敗ではない
        if (FAILED(hr))
            break;
        ...
        {
            // 名前 (CBChangesEveryFrame) を使用して ID3D11ShaderReflectionConstantBuffer インターフェースを取得
            // このインターフェースの他のメソッドを使用して、具体的な情報をさらに照会可能
            // ID3D11ShaderReflectionConstantBuffer は COM コンポーネントではないため、ComPtr に格納できない
            ID3D11ShaderReflectionConstantBuffer* pSRCBuffer = pShaderReflection->GetConstantBufferByName(sibDesc.Name);


            //typedef struct _D3D11_SHADER_BUFFER_DESC {
            //    LPCSTR           Name;		// 定数バッファの名前
            //    D3D_CBUFFER_TYPE Type;		// D3D_CBUFFER_TYPE 列挙値
            //    UINT             Variables;	// 内部変数の数
            //    UINT             Size;		// バッファのバイト数
            //    UINT             uFlags;	    // D3D_SHADER_CBUFFER_FLAGS 列挙値の組み合わせ
            //} D3D11_SHADER_BUFFER_DESC;

            // cbuffer 内の変数情報を取得し、マッピングを構築する
            D3D11_SHADER_BUFFER_DESC cbDesc{};
            hr = pSRCBuffer->GetDesc(&cbDesc);
            if (FAILED(hr))
                return hr;

            // シェーダーのエントリーポイントの uniform パラメータは
            // $Params という名前のデフォルトの定数バッファに属するとみなされる
            bool isParam = !strcmp(sibDesc.Name, "$Params");

            // 定数バッファの作成位置を決定する
            if (!isParam)
            {
                // この方法を使用することで、各定数バッファが一度だけ作成されることを保証する
                // たとえ複数のシェーダーで参照されていても
                auto it = m_CBuffers.find(sibDesc.BindPoint);
                if (it == m_CBuffers.end()) // 見つからなかった場合、新しく作成する
                {
                    // 最初の部分はキー（sibDesc.BindPoint）で、これは定数バッファのバインドポイントを示す。
                    // 第二部分は値で、CBufferData オブジェクトである。
                    // これには定数バッファの名前、開始スロット、データサイズなどの情報が含まれる。
                ...
                // 異なるシェーダー間で cbuffer のサイズが一致しない場合があるため、最大サイズを基準とする必要がある。
                // 例えば、現在のシェーダーではマクロを有効にして cbuffer の最後の変数が有効になり、16 バイト増加したが、
                // 別のシェーダーではその変数が無効になっている場合がある。
                ...
                // このシェーダーが現在の定数バッファを使用していることをマークする
            ...
            for (uint32_t j = 0; j < cbDesc.Variables; ++j)
            {
                ...
                // シェーダーパラメータは特別に処理する必要がある
                // シェーダーのuniformパラメータを記録する
                // **シェーダーパラメータのデフォルト値は無視する**
                if (isParam)
                {
                    switch (shaderFlag)
                    {
            ...
            sibDesc.Type == D3D_SIT_TBUFFER)
        {
            ...
            // 現在のシェーダーリソースがこのシェーダーで使用されていることをマークする
            switch (shaderFlag)
            {
        ...
        else if (sibDesc.Type == D3D_SIT_SAMPLER)
        {
            ...
            // 現在のサンプラーがこのシェーダーで使用されていることをマークする
            switch (shaderFlag)
            {
            ...
            sibDesc.Type == D3D_SIT_UAV_CONSUME_STRUCTURED || sibDesc.Type == D3D_SIT_UAV_RWBYTEADDRESS)
        {
            ...
            // 現在の読み書き可能なリソースがこのシェーダーで使用されていることをマークする
            switch (shaderFlag)
            {
    ...
    ID3DBlob* blob)
{
    ...
    switch (shaderFlag)
    {
     // マクロ展開の例
     //case 0:
     //    // VertexShaderInfo 型のオブジェクトを作成し、そのオブジェクトへのスマートポインタを返す
     //    m_VertexShaders[nameID] = std::make_shared<VertexShaderInfo>();
...
HRESULT EffectHelper::AddShader(std::string_view name, ID3D11Device* device, ID3DBlob* blob)
{
    ...
    // シェーダーのバージョン情報をより扱いやすいフラグ形式に変換する（1を左シフトすることで実現）
...
HRESULT EffectHelper::CreateShaderFromFile(std::string_view shaderName, std::wstring_view filename,
    ID3D11Device* device, LPCSTR enterPoint, LPCSTR shaderModel, const D3D_SHADER_MACRO* pDefines, ID3DBlob** ppShaderByteCode)
{
    ID3DBlob* pBlobIn = nullptr;
    ID3DBlob* pBlobOut = nullptr;
    // シェーダーバイトコードファイルのキャッシュパスが有効であり、強制上書きが無効の場合、
    // 優先的に ${cacheDir}/${shaderName}.cso を読み込み、追加する。
    if (!pImpl->m_CacheDir.empty() && !pImpl->m_ForceWrite)
    {
        std::filesystem::path casheFilename = pImpl->m_CacheDir / (UTF8ToWString(shaderName) + L".cso");
        std::wstring wstr = casheFilename.generic_wstring();
    ...
    // 有効でない、またはキャッシュが存在しない場合、filename を読み込む。
    // もしシェーダーバイトコード（DXBC）であれば、そのまま追加する。
    // コンパイル済みの DXBC ファイルのヘッダーを確認すると、cso ファイル内に "DXBC"（DirectX Bytecode）が含まれていることが分かる。
    static char dxbc_header[] = { 'D','X','B','C' };

    // この段階では、D3DReadFileToBlob 関数はファイルを読み込むために使用されるが、
    // コンパイルや解析処理は行われない。
    HRESULT hr = D3DReadFileToBlob(filename.data(), &pBlobIn);
    if (FAILED(hr))
        return hr;

    // 読み込んだファイルが HLSL ソースコードであるかを判断し、
    // もしそうであればコンパイルを行う。
    // そうでない場合（DXBC 形式のバイトコードである場合）は、
    // そのまま使用する。
    // メモリ比較（メモリ、メモリ、バイト数）
    if (memcmp(pBlobIn->GetBufferPointer(), dxbc_header, sizeof(dxbc_header))) {
        // filename が HLSL ソースコードである場合、コンパイルを行い、追加する。
        // シェーダーバイトコードファイルのキャッシュが有効になっている場合、
        // コンパイルされたシェーダーバイトコードを ${cacheDir}/${shaderName}.cso に保存する。
        // コンパイラに対し、より厳格なエラーおよび警告基準を適用するよう強制する。
        uint32_t dwShaderFlags = D3DCOMPILE_ENABLE_STRICTNESS;
#ifdef _DEBUG
        // D3DCOMPILE_DEBUG フラグを設定すると、シェーダーのデバッグ情報を取得できる。
        // このフラグを有効にすると、デバッグ体験が向上するが、
        // それでもシェーダーの最適化は実行される。
        // 例えば、変数の値を確認したり、コードのどの部分が実行されているかを特定できる。
        dwShaderFlags |= D3DCOMPILE_DEBUG;

        // Debug環境では最適化を無効にして、不合理な動作を回避する。
        // 最適化によりコードの構造が変わる可能性がある。
        // 最適化を無効にすると、コンパイルされたコードがソースコードにより近い形で保持される。
        dwShaderFlags |= D3DCOMPILE_SKIP_OPTIMIZATION;
#endif
        // コンパイル中に発生したエラーや警告メッセージを保存するためのバッファ。
        ...
        hr = D3DCompile(pBlobIn->GetBufferPointer(), pBlobIn->GetBufferSize(), filenameu8str.c_str(),
            pDefines, D3D_COMPILE_STANDARD_FILE_INCLUDE, enterPoint, shaderModel,
    ...
    // 必要であれば、コンパイルされたバイトコードを渡し、不要であれば解放する
    if (ppShaderByteCode)
        *ppShaderByteCode = pBlobOut;
    else
        pBlobOut->Release();
...
HRESULT EffectHelper::CompileShaderFromFile(std::wstring_view filename, LPCSTR entryPoint, LPCSTR shaderModel, ID3DBlob** ppShaderByteCode, ID3DBlob** ppErrorBlob, const D3D_SHADER_MACRO* pDefines, ID3DInclude* pInclude)
{
    uint32_t dwShaderFlags = D3DCOMPILE_ENABLE_STRICTNESS;
#ifdef _DEBUG
    // D3DCOMPILE_DEBUG フラグを設定してシェーダーのデバッグ情報を取得する。
    // このフラグを設定するとデバッグ体験が向上するが、シェーダーの最適化は引き続き可能。
    dwShaderFlags |= D3DCOMPILE_DEBUG;

    // Debug 環境では最適化を無効にし、不合理な動作を防ぐ。
    dwShaderFlags |= D3DCOMPILE_SKIP_OPTIMIZATION;
#endif
...
HRESULT EffectHelper::AddEffectPass(std::string_view effectPassName, ID3D11Device* device, const EffectPassDesc* pDesc)
{
    ...
    // マクロ展開
    //if(!pDesc->nameVS.empty())
    //{
    //    auto it = pImpl->m_VertexShaders.find(StringToID(pDesc->nameVS));
    //    if (it != pImpl->m_VertexShaders.end())
    //    {
    //        pEffectPass->pVSInfo = it->second;
    //        // デフォルトの定数バッファに対する操作がある場合
    //        auto& pCBData = it->second->pParamData;
    //        if (pCBData) 
    //        {
    //            pEffectPass->pVSParamData = std::make_unique<CBufferData>(pCBData->cbufferName.c_str(),
    //                pCBData->startSlot, (uint32_t)pCBData->cbufferData.size());
    //            it->second->pParamData->CreateBuffer(device);
    //        }
    //    }
    //    else
...
{
    //
    // シェーダー、定数バッファ、パラメータ定数バッファ、サンプラー、シェーダーリソース、読み書き可能リソースを設定
    //
    if (pVSInfo)
    {
        //    //deviceContext->VSSetShader(pVSInfo->pVS.Get(), nullptr, 0);
        EFFECTPASS_SET_SHADER(VS);

        //    uint32_t slot = 0, mask = pVSInfo->cbUseMask;
        //    while (mask){
        //        if ((mask & 1) == 0) {
        //            ++slot, mask >>= 1;
        //            continue;
        //        }
        //        uint32_t zero_bit = ((mask + 1) | mask) ^ mask;
        //        uint32_t count = (zero_bit == 0 ? 32 : (uint16_t)log2((double)zero_bit));
        //        if (count == 1) {
        //            CBufferData& cbData = cBuffers.at(slot);
        //            cbData.UpdateBuffer(deviceContext);
        //            deviceContext->VSSetConstantBuffers(slot, 1, cbData.cBuffer.GetAddressOf());
        //            ++slot, mask >>= 1;
        //        }
        //        else {
        //            std::vector<ID3D11Buffer*> constantBuffers(count);
        ...
        //                constantBuffers[i] = cbData.cBuffer.Get();
        //            }
        //            deviceContext->VSSetConstantBuffers(slot, count, constantBuffers.data());
        //            slot += count + 1, mask >>= (count + 1);
        //        }
        //    }
        ...
        //if (!pVSInfo->params.empty()) 
        //{
        //    if (pVSParamData->isDirty)
        //    {
        //        pVSParamData->isDirty = false;
        //        pVSInfo->pParamData->isDirty = true;
        //        memcpy_s(pVSInfo->pParamData->cbufferData.data(), pVSParamData->cbufferData.size(),
        //            pVSParamData->cbufferData.data(), pVSParamData->cbufferData.size());
        //        pVSInfo->pParamData->UpdateBuffer(deviceContext);
        //    }
        //    deviceContext->VSSetConstantBuffers(pVSInfo->pParamData->startSlot,
        //        1, pVSInfo->pParamData->cBuffer.GetAddressOf());
        //}
        EFFECTPASS_SET_PARAM(VS);

        //uint32_t slot = 0, mask = pVSInfo->ssUseMask;
        //while (mask) {
        //    if ((mask & 1) == 0) {
        //        ++slot, mask >>= 1;
        //        continue;
        ...
        //    uint32_t count = (zero_bit == 0 ? 32 : (uint32_t)log2((double)zero_bit));
        //    if (count == 1) {
        //        deviceContext->VSSetSamplers(slot, 1, samplers.at(slot).pSS.GetAddressOf());
        //        ++slot, mask >>= 1;
        //    }
        //    else {
        //        std::vector<ID3D11SamplerState*> samplerStates(count);
        //        for (uint32_t i = 0; i < count; ++i)
        //            samplerStates[i] = samplers.at(slot + i).pSS.Get();
        //        deviceContext->VSSetSamplers(slot, count, samplerStates.data());
        //        slot += count + 1, mask >>= (count + 1);
        //    }
        //}
        ...
        //uint32_t slot = 0;
        //for (uint32_t i = 0; i < 4; ++i, slot = i * 32) {
        //    uint32_t mask = pVSInfo->srUseMasks[i];
        //    while (mask) {
        //        if ((mask & 1) == 0) {
        //            ++slot, mask >>= 1;
        //            continue;
        ...
        //        uint32_t count = (zero_bit == 0 ? 32 : (uint32_t)log2((double)zero_bit));
        //        if (count == 1) {
        //            deviceContext->VSSetShaderResources(slot, 1, shaderResources.at(slot).pSRV.GetAddressOf());
        //            ++slot, mask >>= 1;
        //        }
        //        else {
        //            std::vector<ID3D11ShaderResourceView*> srvs(count);
        //            for (uint32_t i = 0; i < count; ++i)
        //                srvs[i] = shaderResources.at(slot + i).pSRV.Get();
        //            deviceContext->VSSetShaderResources(slot, count, srvs.data());
        //            slot += count + 1, mask >>= (count + 1);
        //        }
        //    }
        //}
    ...
    if (pPSInfo)
    {
        ...
        if (pPSInfo->rwUseMask)
        {
            ...
            // 必ず一括設定する必要がある。1つでもカウンターの初期化が必要であれば、すべてが初期化される。
            deviceContext->OMSetRenderTargetsAndUnorderedAccessViews(
                D3D11_KEEP_RENDER_TARGETS_AND_DEPTH_STENCIL, nullptr, nullptr,
                firstSlot,
                (uint32_t)pUAVs.size() - firstSlot,
                &pUAVs[firstSlot],
