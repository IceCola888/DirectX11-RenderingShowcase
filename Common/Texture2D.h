...
protected:
    ComPtr<ID3D11Texture2D> m_pTexture;             // TextureCube 作成時に転送の役割を果たす
    ComPtr<ID3D11ShaderResourceView> m_pTextureSRV; // TextureCube 作成時にテクスチャ配列の SRV として使用
...
public:
    ...
    void SetMsaaSamples(uint32_t msaa) { m_MsaaSamples = msaa; }
...
class Depth2DCube :public Texture2DBase
{
public:
    Depth2DCube(ID3D11Device* device, uint32_t width, uint32_t height,
        uint32_t mipLevels = 1,
        DepthStencilBitsFlag depthStencilBitsFlag = DepthStencilBitsFlag::Depth_24Bits_Stencil_8Bits,
        uint32_t bindFlags = D3D11_BIND_DEPTH_STENCIL | D3D11_BIND_SHADER_RESOURCE);
    ~Depth2DCube()override = default;
