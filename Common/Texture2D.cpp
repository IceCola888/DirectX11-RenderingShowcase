    ...
    : m_Width(texDesc.Width), m_Height(texDesc.Height)
{
    ...
    // 実際に生成されたMIPレベルなどのデータを更新する
    D3D11_TEXTURE2D_DESC desc;
    m_pTexture->GetDesc(&desc);
    m_MipLevels = desc.MipLevels;

    // 完全なリソースのSRV（Shader Resource View）
...
TextureCube::TextureCube(ID3D11Device* device, uint32_t width, uint32_t height,
    ...
    : Texture2DBase(device,
        CD3D11_TEXTURE2D_DESC(format, width, height, 6, mipLevels, bindFlags, D3D11_USAGE_DEFAULT,
            0, 1, 0, D3D11_RESOURCE_MISC_TEXTURECUBE | D3D11_RESOURCE_MISC_GENERATE_MIPS),
...
Depth2DCube::Depth2DCube(ID3D11Device* device, uint32_t width, uint32_t height,
    uint32_t mipLevels, DepthStencilBitsFlag depthStencilBitsFlag, uint32_t bindFlags)
    :Texture2DBase(device,
        CD3D11_TEXTURE2D_DESC(GetDepthTextureFormat(depthStencilBitsFlag), width, height, 6, 1, bindFlags, D3D11_USAGE_DEFAULT,
            0, 1, 0, D3D11_RESOURCE_MISC_TEXTURECUBE),
        CD3D11_SHADER_RESOURCE_VIEW_DESC(D3D11_SRV_DIMENSION_TEXTURECUBE, GetDepthSRVFormat(depthStencilBitsFlag)))
{
    if (bindFlags & D3D11_BIND_DEPTH_STENCIL)
    {
        for (uint32_t i = 0; i < 6; ++i)
...
void Depth2DCube::SetDebugObjectName(std::string_view name)
{
    Texture2DBase::SetDebugObjectName(name);
#if (defined(DEBUG) || defined(_DEBUG)) && (GRAPHICS_DEBUGGER_OBJECT_NAME)
    for (size_t i = 0; i < m_pDepthStencilElements.size(); ++i)
        ::SetDebugObjectName(m_pDepthStencilElements[i].Get(), std::string(name) + ".RTV[" + std::to_string(i) + "]");
    for (size_t i = 0; i < m_pShaderResourceElements.size(); ++i)
        ::SetDebugObjectName(m_pShaderResourceElements[i].Get(), std::string(name) + ".SRV[" + std::to_string(i) + "]");
#else
    UNREFERENCED_PARAMETER(name);
